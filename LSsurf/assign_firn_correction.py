...
def assign_firn_correction(data, firn_correction, hemisphere, subset_valid=True, EXTRAPOLATE=False):
    ...
    if firn_correction == 'MAR':
        ...
        if hemisphere==1:
            h_firn=SMBcorr.interpolate_racmo_firn('/Volumes/ice1/tyler', "EPSG:3413", 'FGRN055', data.time, data.x, data.y, VARIABLE='zs', FILL_VALUE=np.NaN)[0]
            # check if any data points are over "unglaciated" values
            bad=~np.isfinite(h_firn)
            if np.any(bad):
                # extrapolate model to coordinates
                h_firn[bad]=SMBcorr.extrapolate_racmo_firn('/Volumes/ice1/tyler', "EPSG:3413", 'FGRN055', data.time[bad], data.x[bad], data.y[bad], VARIABLE='zs', FILL_VALUE=np.NaN)[0]
        ...
        if hemisphere==1 and firn_correction in ["RACMO_fac","RACMO_fac_smb"]:
            h_fac = SMBcorr.interpolate_racmo_firn('/Volumes/ice1/tyler', "EPSG:3413", 'FGRN055', data.time, data.x, data.y, VARIABLE='FirnAir', FILL_VALUE=np.NaN)[0]
            # check if any data points are over "unglaciated" values
            bad=~np.isfinite(h_fac)
            if np.any(bad):
                # extrapolate model to coordinates
                h_fac[bad]=SMBcorr.extrapolate_racmo_firn('/Volumes/ice1/tyler', "EPSG:3413", 'FGRN055', data.time[bad], data.x[bad], data.y[bad], VARIABLE='FirnAir', FILL_VALUE=np.NaN)[0]
            data.z -= h_fac
        ...
        if firn_correction in ["RACMO_fac_smb","RACMO_smb"]:
            ...
            rho_model = 910.0
            if hemisphere==1:
                smb = SMBcorr.interpolate_racmo_downscaled('/Volumes/ice1/tyler', "EPSG:3413", '3.0', 'SMB', data.time, data.x, data.y, FILL_VALUE=np.NaN)[0]
