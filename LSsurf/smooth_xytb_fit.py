...
def iterate_fit(data, Gcoo, rhs, TCinv, G_data, Gc, in_TSE, Ip_c, timing, args):
    ...
    for iteration in range(args['max_iterations']):
        ...
        sigma_extra=calc_sigma_extra(r_data[in_TSE], data.sigma[in_TSE])
        ...
        if iteration >= 2:
            if sigma_hat <= 1:
                ...
                break
    return m0, sigma_extra, in_TSE, in_TSE_last, rs_data
...
def smooth_xytb_fit(**kwargs):
    ...
    if args['max_iterations'] > 0:
        tic_iteration=time()
        m0, sigma_extra, in_TSE, in_TSE_last, rs_data=iterate_fit(data, Gcoo, rhs, \
